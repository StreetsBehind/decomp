...
tags: [synthesis, M5, fit, decision]
...
"…and an **automated search** can find those harness setups better than a human tuning by hand," and its
**M5** is "an adaptive, self-tuning harness… finding combinations a human wouldn't hand-tune, which we
then *explain*." The field surveyed in [methods](/methods/index.md) is the toolbox for that M5.

# The mapping
The [evolutionary frame](/concepts/evolutionary-frame.md) maps cleanly onto the existing apparatus, so it
*describes* the program more than it extends it:

| Frame | This program |
|---|---|
| Genes | the lever menu (verification, retries, sub-decompose, skeleton, routing) + knobs (skeleton shape/obligations split, retry, ~1-surface chunk granularity, transport) |
| Chromosome | a `strategy` behind the adapter contract |
| Calibration | the battery (`strategy × fixture × K`) |
...
[2] docs/REPORT-2026-06-16.md §10–11 (settled vs open; the MoE/ensemble slot)
