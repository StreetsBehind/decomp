...
export async function runSingleSession(fixture, ctx, { arm = 'blind', armBlock = '' } = {}) {
  ...
  // GRANULARITY KNOB (RESEARCH-PROGRAM §4.2): when the runner threads ctx.granularity, the level's
  // clause joins the contract AND the deterministic post-pass enforces it — shared by all 3 arms.
  ...
  const prompt = [
    ...
    '',
    snapshotContract(outcomeIds, { openQuestions: true, granularity }),
  ].join('\n');
  ...
  const { snapshot: parsed, gaps } = parseReply(res.text);
  // Enforce the dose mechanically (free, deterministic) so the knob bites even when the model
  // ignores the clause; the MEASURED granularity is recorded by the runner either way.
