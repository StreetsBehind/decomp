...
- **Strengthen the placebo** — equal *tokens* ≠ equal *structure*. Consider a 4th arm: a plan-specific but **deliberately mismatched** checklist (e.g. treat the SSO feature as a CRUD resource). If A2 beats *that*, the typed archetype *content* is doing work; if it ties, you measured "concreteness," not archetypes.
...
3. ~~**Build §7.1–§7.6.**~~ ✅ **DONE** (2026-06-02) — primed/placebo arms, C1 lint, partition scorer, AND §7.2's live annotation pass + Step-2 A/B all built + self-tested (**238 selftest assertions**; `battery:mock` green). The full apparatus exists. **What now gates a live run is methodology/content, not code:** the **placebo length-match** (§8), registering the multi-feature **`hearth`** fixture (second-author its partitioned manifest + lock + planted-gaps, then hash-pin), and the **manipulation-check + headroom pre-flight** so a null is interpretable.
